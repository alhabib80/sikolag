...
class Administrator extends CI_Controller {
	...
	function edit_halamanbaru(){
		...
		if (isset($_POST['submit'])){
            ...
            //return false;
            //ambil gambar lama
            $where1 = array('id_halaman' => $this->input->post('id'));
            $data['iden'] = $this->model_app->view_where('halamanstatis',$where1)->row_array();
            //echo $data['iden']['gambar'];
            $hasil = '';
            if($data['iden']['gambar'] != null || $data['iden']['gambar'] !=''){
                if($_FILES['ca']['name'] != ''){
                    if(file_exists($upload_path.'/'.$temp->ELEMENT_URL)){
                        $new_name = $this->input->post('id')."_".$_FILES["ca"]['name'];
                        $config['file_name'] = $new_name;
                        $path_to_file = 'asset/foto_statis/'.$data['iden']['gambar'];
                        unlink($path_to_file);
                    } else {
                        $new_name = $this->input->post('id')."_".$_FILES["ca"]['name'];
                        $config['file_name'] = $new_name;
	...
	function delete_halamanbaru(){
        ...
        //die($id['id_halaman']);
        $where1 = array('id_halaman' => $id['id_halaman']);
        $data['iden'] = $this->model_app->view_where('halamanstatis',$where1)->row_array();

        if($data['iden']['gambar'] != null || $data['iden']['gambar'] !=''){
            if(file_exists('asset/foto_statis/'.$data['iden']['gambar'])){
                $path_to_file = 'asset/foto_statis/'.$data['iden']['gambar'];
                unlink($path_to_file);
            }
        }
